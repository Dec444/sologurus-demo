...
test("community finder filters opt-in profiles by language, location, and miles", async () => {
  const coordinates = { latitude: "10.7769", longitude: "106.7009" };
  const fiveMileResponse = await search({ language: "English", location: "Ho Chi Minh City, Vietnam", radius: "5", ...coordinates });
  ...
  assert.equal(fiveMile.directoryMode, "real-location-preview-directory");
  assert.deepEqual(fiveMile.coordinatesUsed, { latitude: 10.7769, longitude: 106.7009 });
  ...
  const widerResponse = await search({ language: "English", location: "Ho Chi Minh City, Vietnam", radius: "25", ...coordinates });
